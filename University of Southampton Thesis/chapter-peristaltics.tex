...
using soft lithography. \fig{fig:Ungar} shows the devices, these work by having a fluid path that has various gas
...
\begin{figure}
  \begin{center}
  \includegraphics[width=\textwidth,height=4cm,keepaspectratio]{UngarFig.png}
  \end{center}
...
With all these limitations another material one might suggest is PDMS. It’s cheap and
...
The solution employed here, shown in \fig{fig:3Ddevice}, involves a multilayered PMMA device
...
In order for the dead volume in the device to be kept at a minimum, the valves should be implemented in
the fluid path on the device itself. Shown below in \fig{fig:PP-device} is the
...
A 3D rendering of a single valve is shown in \fig{fig:ValveRend} and shows the ratio of pump holder opening, top hole; and middle hole respecively. The central principle of these valves
...
In \fig{fig:Micrographs}, micrographs of the chip outside the holders are shown with the valves where one can see the fluid channels on opposing side of their respective layers. Also given, is side by side comparison of the same valve (valve 2 in \fig{fig:Chip}) open (2) and closed (3) one can see the 'ring' formed by the PDMS as it seals against the middle layer.
...
\subsubsection{Characterisation of flow}
...
\fig{fig:Chip}. This left C to be “in” and D to be the “out” port with valves 3, 2, and 1
...
\begin{figure}
  \begin{center}
  \includegraphics[width=\columnwidth]{Flowratevsfreq.pdf}
...
frequencies. When initially observing the slope of the lines in \fig{fig:Graph} the non-
...
\begin{figure}
  \begin{center}
  \includegraphics[width=\columnwidth]{Acetate-DSS-stackplot-edit.pdf}
...
the inner circuit by attaching a syring to inlet \textbf{B} in \fig{fig:Chip}. 100mM DSS in DI
