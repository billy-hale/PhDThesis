...
from \eqn{eqn:zeeman}, inner products of the overall state, $\ket{\psi}$ with $\ket{\alpha}$
...
In \eqn{eqn:expectation}, the expectation value of an operator was expressed as
...
\end{pmatrix},
\end{equation}
using the definition given in \eqn{eqn:operators} the density operator can be re-written as
\begin{equation}
...
The sandwich equation can be used to calculate the effect of a strong $(\pi/2)_x$ pulse
...
This magnetization leads to the signal obtained in NMR, to find the relationship the principle of reciprocity
...
\end{equation}
Substitution of the result from \eqn{eqn:magnetization} gives
\begin{equation}
...
\end{equation}
using \eqn{eqn:MagneticMoment}, $\langle\hat{\mu}_x\rangle = \hbar\gamma\langle{\hat{I}_x}\rangle$
the emf becomes
\begin{equation}
...
the density operator must be calculated first. Using definitions from \eqn{eqn:1Def}
and \eqn{eqn:BDef}, the rotating frame density operator at equilibrium is
\begin{equation}
...
a smaller portion of spins in the sample, \hl{where the magnetic field could be more homogeneous}, but this compromises sensitivity as the number of spins become more limited.
 ...
 at thermal equilibrium, $a$, can be expressed as \eqn{eqn:SignalAmplitude}:
\begin{equation}
 a = \frac{1}{4}\frac{B_{1x}}{i_c}\gamma^3\hbar^2B_0^2\frac{n_s}{k_bT},
\end{equation}
...
the amplitude from \eqn{eqn:SignalAmplitude} and using the \textit{Rayleigh-Jeans approxiamation} for the noise the SNR is:
...
al.} and for an equivalent helix \hl{can} give $\sqrt{2}$ larger SNR \citep{vanBentum:2007fda}. The TLP geometry, shown in \fig{fig:MVProbe},
...
\begin{figure}[ht]
  \begin{center}
  \includegraphics[height=11cm,keepaspectratio]{Manvendra-Probe.jpg}
  \end{center}
